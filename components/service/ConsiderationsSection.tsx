...
export default function ConsiderationsSection({
  ...
  return (
    <SectionShell tone={tone}>
      ...
      <div className="mt-11 grid gap-x-12 gap-y-8 md:grid-cols-2">
        {items.map((item, i) => (
          <Reveal key={item.title} delay={Math.min(i, 4) * 55}>
            <div>
              <h3 className="m-0 mb-[10px] flex items-baseline gap-3 font-serif text-[18px] font-normal text-[#1B2430]">
                <span
                  aria-hidden="true"
              ...
              <p className="m-0 pl-[19px] text-[15.5px] leading-[1.7] text-[#4B5563]">
                {item.desc}
              </p>
            </div>
          </Reveal>
      ...
      {disclaimer ? (
        <Reveal>
          <p className="m-0 mt-11 max-w-[820px] border-t border-[#E5E4E0] pt-6 text-[14.5px] leading-[1.7] text-[#6B7280]">
            {disclaimer}
          </p>
        </Reveal>
      ) : null}
